...
NBOnly <- dplyr::select(NBOnly, c(Gender, MYCN, Age_Diagnosis, Vital_Status, Diagnostic_Category, Overall_Survival_Time_in_Days, TARGET_USI)) #Filter data for only necessary variables
...
##Exploring, Visualizing, and Testing Questions
...
```
Based on the table, it appears that patients with Amplified MYCN have a higher proprtion of Dead individuals than patients with Not Amplified MYCN.
...
As we can see from the ouput above, we get a X-squared value of 0.003018 and a p-value is less than 0.05 which means that we reject the null hypothesis and can see that the two variables are in fact dependent on one another! 
