...
**The y-axis is weird because of the Unkown bar and idk how to get rid of it/ if we need to get rid of it because we have the key that makes it clear to read**
...
First we can create a basic boxplot. 
...
```{r}
amp.chitest <- table(NBOnly$MYCN, NBOnly$Vital_Status)
amp.chitest
chisq.test(amp.chitest)
```

We get a X-squared value of 9.5334 and the p-value is less than 0.05 which means that we reject the null hypothesis and can see that the two variables are in fact dependent on one another!
...
probably same type of analysis as question 1-- could be good maybe if one is and one isn't so we could have varying examples but however it comes out is obviously good
